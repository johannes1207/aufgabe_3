...
public class ManualTestBean {
    ...
    public void callController(){
        ...
        log.info("----------------------------------------");
        log.info("-------------  Teilaufgabe a  ---------------");
        // 1. Schritt Identifikation des konkreten Items
        ...
        if(potentialMovies.size() == 1){
           //2. Nur wenn Suchanfrage eindeutig wird die Entität geändert.
        ...
        //3. Überprüfung der Änderung
        log.info("----- Test Query 3: Check modifications -----");
        this.movieRepository.findByTitleContaining("Star").
                forEach(movie->log.info(movie.getTitle()));

        log.info("-------------  Teilaufgabe b  ---------------");
        this.movieRepository.findByPremiereDateAfter(LocalDate.of(2015,1,1)).
                forEach(movie->log.info(movie.getTitle()));

        log.info("-------------  Teilaufgabe c  ---------------");
        this.movieRepository.findAllShortMoviesWithGoodImdbRating().
                forEach(movie->log.info(movie.getTitle()));
